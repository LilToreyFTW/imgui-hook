...
// proxine.lib removed - using local stubs in auth.hpp to eliminate unauditable external binary
	...
	void InitImGui()
{
	...
	Menu::BiggestIcon = defaultFont;
	Menu::BiggestFont = defaultFont;
	Menu::littleFont = defaultFont;
#else
...
HRESULT __stdcall PresentHook(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
	if (Menu::FirstTime)
	{
		if (!BindD3DInfo(pSwapChain))
			return DirectX::OriginalPresent ? DirectX::OriginalPresent(pSwapChain, SyncInterval, Flags) : S_OK;
	}
	...
	// INSERT or F5 - some laptops don't have INSERT
	if ((SAFE_CALL(GetAsyncKeyState)(VK_INSERT) & 1) || (SAFE_CALL(GetAsyncKeyState)(VK_F5) & 1))
	{
		Menu::Open = !Menu::Open;
	}
	...
	if (AuthConnected)
	{
		bool gameValid = (FiveM::World != 0 && FiveM::W2S != 0 && FiveM::BonePos != 0);
		if (gameValid) {
			Visuals::Hook();
			Players::Hook();
		...
		if (gameValid && Settings::Aimbot::Draw_Fov)
		{
			...
			if (ImGui::Begin(E("Auth"), nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar))
			{
				...
				if (Tabs == 2)
				{
					...
					if (ImGui::Button(E("Login##Log"), ImVec2(320, 25)))
					{
						...
						ImGui::InsertNotification(std::make_tuple((int)ImGuiToastType_Success, 3000, std::string("Connecting.. :)")));
	...
	if (DirectX::pContext && DirectX::renderTargetView) {
...
bool Core::Init()
{
	...
	while (!DirectX::OverlayHooked)
	{
		if (Hook::Init())
		{
			DirectX::OverlayHooked = Hook::Present((void**)&DirectX::OriginalPresent, (void*)PresentHook);
		}
	}
